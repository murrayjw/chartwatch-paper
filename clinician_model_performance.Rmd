...
threshold <- FINAL_PAPER_ENSEMBLE_THRESHOLD
test_encounter_nums <- read.csv(FINAL_PAPER_TEST_ENCOUNTERS_FILENAME,
                                stringsAsFactors = FALSE) %>%
  dplyr::pull(ENCOUNTER_NUM)
all_clinician_predictions <- read.csv(FINAL_PAPER_TEST_CLINICIAN_PREDICTIONS_FILENAME,
                                      stringsAsFactors = FALSE) %>%
...
clinician_predictions <- all_clinician_predictions %>%
  dplyr::filter(ENCOUNTER_NUM %in% test_encounter_nums)

model_predictions <- read.csv(FINAL_PAPER_TEST_TIME_AWARE_PREDICTIONS_FILENAME,
                              stringsAsFactors = FALSE) %>%
  dplyr::filter(ENCOUNTER_NUM %in% test_encounter_nums)
...
Initially, there are `r nrow(all_clinician_predictions)` predictions on `r length(unique(all_clinician_predictions$ENCOUNTER_NUM))` visits and `r length(unique(all_clinician_predictions$MRN))` patients, from `r length(unique(all_clinician_predictions$clinicianid))` clinicians. The predictions are done between `r as.Date(min(all_clinician_predictions$timestamp))` and `r as.Date(max(all_clinician_predictions$timestamp))`.

```{r, echo = F}
all_clinician_predictions %>% 
  dplyr::summarize(
    start = as.Date(min(timestamp)),
    end = as.Date(max(timestamp))
  ) %>%
  knitr::kable(caption = "Start and end date of prospective test data")

all_clinician_predictions %>%
  dplyr::summarize(
    num_predictions = n(),
    num_clinicians = n_distinct(clinicianid),
    num_encounters = n_distinct(ENCOUNTER_NUM)
  ) %>%
  knitr::kable(caption = "All clinician predictions")

tableone::CreateTableOne(data = all_clinician_predictions,
                         vars = c("professional_group", "outcome", "outcome48"),
                         factorVars = c("professional_group", "outcome", "outcome48")) %>%
  tableone::kableone(caption = "All clinician predictions")


```

We then filter predictions to **only include visits that started during the prospective test period**. This results in `r nrow(clinician_predictions)` predictions on `r length(unique(clinician_predictions$ENCOUNTER_NUM))` visits and `r length(unique(clinician_predictions$MRN))` patients, from `r length(unique(clinician_predictions$clinicianid))` clinicians. The predictions are done between `r as.Date(min(clinician_predictions$timestamp))` and `r as.Date(max(clinician_predictions$timestamp))`.
...
clinician_predictions %>% 
  ...
  knitr::kable(caption = "Start and end date of prospective test data -- after filtering")
...
clinician_predictions %>%
  ...
  knitr::kable(caption = "Clinician predictions after filter applied")

tableone::CreateTableOne(data = clinician_predictions,
                         vars = c("professional_group", "outcome", "outcome48"),
                         factorVars = c("professional_group", "outcome", "outcome48")) %>%
  tableone::kableone(caption = "Clinician predictions after filter applied")

encounter_level_outcomes <- clinician_predictions %>%
...
tableone::CreateTableOne(data = encounter_level_outcomes,
  ...
  tableone::kableone(caption = "Outcomes by encounter")
