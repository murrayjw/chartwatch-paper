...
train_data <- read.csv(FINAL_PAPER_TRAIN_ENCOUNTERS_DATA_FILENAME, stringsAsFactors = FALSE)
train_encounters <- read.csv(FINAL_PAPER_TRAIN_ENCOUNTERS_FILENAME, stringsAsFactors = FALSE)
train_vitals <- read.csv(FINAL_PAPER_TRAIN_VITALS_DATA_FILENAME, stringsAsFactors = FALSE)

valid_data_method2 <- read.csv(FINAL_PAPER_VALID_ENCOUNTERS_DATA_FILENAME, stringsAsFactors = FALSE)
valid_encounters_method2 <- read.csv(FINAL_PAPER_VALID_ENCOUNTERS_FILENAME, stringsAsFactors = FALSE)
valid_vitals_method2 <- read.csv(FINAL_PAPER_VALID_VITALS_DATA_FILENAME, stringsAsFactors = FALSE)

test_data_method2 <- read.csv(file.path(data_folder, "test_encounters_data_method1.csv"), stringsAsFactors = FALSE)
test_encounter_nums_method2 <- test_data_method2 %>% pull(ENCOUNTER_NUM)
# Gives dataframe with 1 outcome/encounter
test_encounters_method2 <- read.csv(file.path(data_folder, "test_encounters_with_outcomes_method1.csv"), stringsAsFactors = FALSE) 
# Gives dataframe with potentially multiple outcomes/encounter (e.g., encounter could transfer to ICU and then die)
test_outcomes_method2 <- readr::read_csv(FINAL_PAPER_TEST_CLINICIAN_PREDICTIONS_FILENAME) %>%
...
# Time period of each cohort

```{r echo = FALSE, message = FALSE}
test_data_ts <- test_data_method2 %>%
  dplyr::filter(ENCOUNTER_NUM %in% test_encounter_nums_method2)
dplyr::tibble(
  "Cohort" = c("Train", "Validation", "Test"),
  "Start date" = c(min(train_data$ADMIT_TS), min(valid_data_method2$ADMIT_TS), min(test_data_ts$ADMIT_TS)),
  "End date" = c(max(train_data$ADMIT_TS), max(valid_data_method2$ADMIT_TS), max(test_data_ts$ADMIT_TS))
) %>%
  knitr::kable()
```
...
tableone::CreateTableOne(data = train_encounters, 
  ...
  print(nonnormal = c("gim_to_outcome"), 
                     showAllLevels = TRUE) 
...
Method 1: Keep all test encounters. 
...
# Outcomes - Compute: 
#  - % of encounters that experience any outcome
#  - % of encounters that experience death
#  - % of encounters that experience ICU
#  - % of encounters that experience palliative transfer
encounter_level_outcomes <- test_outcomes_method2 %>%
...
tableone::CreateTableOne(data = encounter_level_outcomes,
                         vars = c("outcome", "death", "icu", "palliative"),
                         factorVars = c("outcome", "death", "icu", "palliative"))
